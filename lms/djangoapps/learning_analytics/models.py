...
# Model for data to be used on 'Video time distribution'
...
# Codigo Javier Orcoyen
# Model for data to be used on 'Video time watched' chart
# Related to time spent on videos. 
# Total time and percent viewed
...
class VideoIntervals(models.Model):
    ...
    interval_start = models.TextField(db_index=False)
    interval_end = models.TextField(db_index=False)
    ids = models.TextField(db_index=False)
...
# Codigo Javier Orcoyen
# Related to the last known tracking log, but it will only store id, student key, course key and module_key
