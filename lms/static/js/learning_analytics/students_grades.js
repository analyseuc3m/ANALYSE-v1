	...
	var drawChart = function() {
		...
		if(data == null){
			...
			if(all_sections == null){
			    ...
			    document.getElementById('students_grades_chart').innerHTML = "";
			    document.getElementById('students_grades_chart').appendChild(noData);
			}else{
		...
		var chart = new google.visualization.BarChart(document.getElementById('students_grades_chart'));
	...
	var setRowData = function(row){
		...
		for(var i = 0; i < gs.length; i++){
			if (isTotal || gs[i]['category'] == category){
				...
				cat_array.push([gs[i]['label'], percent*100, color]);
			}
		}
